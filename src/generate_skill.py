...
from src.utils.list_dict_operations import mix_list
...
def main(confidence: float):
    ...
    mixed_entities = mix_list(skill["entities"], new_entities, key_priority)
    ...
    removed_intents = [
        intent["intent"]
        for intent in mixed_intents
        if intent["intent"] not in used_intents
    ]
    mixed_intents = [
    ...
    mixed_skill = mix_skills(
        skill, intents=mixed_intents, entities=mixed_entities, dialog_nodes=mixed_nodes,
    )
